use crate::part::{Primitives, Conserved};
...
impl RiemannSolver {
    ...
    pub fn solve_for_flux(&self, left: &Primitives, right: &Primitives) -> Conserved {
        ...
        let a_l = (self.gamma * left.pressure() * rho_l_inv).sqrt();
        let a_r = (self.gamma * right.pressure() * rho_r_inv).sqrt();
        ...
        let s_l = left.velocity() - a_l * q_l;
        let s_r = right.velocity() + a_r * q_r;
        let s_l_m_v_l = s_l - left.velocity();
        let s_r_m_v_r = s_r - right.velocity();
