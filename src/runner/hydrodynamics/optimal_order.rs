use crate::{
    finite_volume_solver::FiniteVolumeSolver, gravity::GravitySolver, timeline::IntegerTime,
    ParticleMotion, Space, TimestepInfo,
};

use super::{
    apply_fluxes, gradient_apply, gradient_estimate, gradient_limit, kick1, kick2,
    reset_extrapolations, timestep_limiter, timestep_sync, timesteps_apply,
};
use crate::hydrodynamics::{apply_flux_limiter, flux_limiter_collect};
...
impl Runner for OptimalOrderRunner {
    ...
    fn step(
        &self,
        space: &mut Space,
        fv_solver: &Box<dyn FiniteVolumeSolver>,
        ...
        if fv_solver.do_flux_limit() {
            // Gather flux limiter info
            apply_flux_limiter(
                space,
                &flux_limiter_collect(space, &part_is_active, fv_solver),
            );
        }

        // Fluxes
        let fluxes = fv_solver.compute_fluxes(
            space.faces(),
            space.parts(),
            &part_is_active,
            space.boundary(),
        ...
        let mut timesteps = fv_solver.compute_timesteps(
            space.parts_mut(),
            &part_is_active,
            particle_motion,
            dimensionality,
        ...
        if let Some(gravity_solver) = gravity_solver {
            ...
            timesteps
                .par_iter_mut()
                .zip(timesteps_grav.par_iter())
                .for_each(|(dt_fv, dt_grav)| *dt_fv = dt_fv.min(*dt_grav));
        }
        let ti_next = timestep_info.ti_current
            + timesteps_apply(space, &timesteps, &part_is_active, timestep_info);
